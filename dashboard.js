/* ═══════════════════════════════════════════
   MEATMASTER — dashboard.js  v2.2  (FIXED)
   ═══════════════════════════════════════════ */
...
async function renderCustomerDash() {
  ...
  // Fetch butcher profiles separately to avoid FK join issues
  const butcherIds = [...new Set((bookings||[]).map(b => b.butcher_id).filter(Boolean))];
  let butcherMap = {};
  if (butcherIds.length) {
    const { data: profData } = await sb
      .from('profiles')
      .select('id, full_name, city')
      .in('id', butcherIds);
    (profData || []).forEach(p => { butcherMap[p.id] = p; });
  }
  ...
  wrap.innerHTML = `
    <div class="dash-topbar">
      ...
      <button class="btn btn-teal-outline" onclick="showPage('home');loadButchers()">
    ...
    <div class="kpi-row">
      ...
      <div class="kpi green"><div class="kpi-label">${t('Accepted','قبول شدہ')}</div><div class="kpi-val">${accepted}</div></div>
      <div class="kpi soft"><div class="kpi-label">${t('Completed','مکمل')}</div><div class="kpi-val">${completed}</div></div>
    </div>
    <div class="panel">
      <div class="panel-title">${t('My Bookings','میری بکنگز')}</div>
      ${buildCustomerRows(bookings, butcherMap)}
    </div>`;
}

function buildCustomerRows(bookings, butcherMap) {
  ...
  return bookings.map(b => {
    const prof  = butcherMap?.[b.butcher_id] || {};
    const bname = prof.full_name || t('Unknown Butcher','نامعلوم قصائی');
    const bcity = prof.city || '';
...
async function renderButcherDash() {
  ...
  // FIX: fetch butcher_profiles without RLS filter (butcher reads own row)
  const [bpRes, bookRes, rateRes] = await Promise.all([
    sb.from('butcher_profiles').select('*').eq('id', currentUser.id).maybeSingle(),
    sb.from('bookings').select('*, customer_id').eq('butcher_id', currentUser.id).order('created_at', { ascending: false }),
  ...
  if (custIds.length) {
    const { data: custData } = await sb
      ...
      .in('id', custIds);
    (custData || []).forEach(p => { custMap[p.id] = p; });
  }
  ...
  wrap.innerHTML = `
      ...
      <div>
        <div class="dash-greeting">
          ...
          <span class="dash-role-tag">
            ${bp.is_verified ? '✓ '+t('Verified','تصدیق شدہ') : '⏳ '+t('Butcher','قصائی')}
          </span>
        </div>
    ...
    <div class="panel">
      ...
      <div class="avail-row" id="availRow" onclick="toggleAvail(${!avail})">
        ...
        <span class="avail-label" id="tglLabel">
          ${avail ? t('✅ Available for new bookings','✅ نئی بکنگز کے لیے دستیاب')
    ...
    <div class="panel">
      <div class="panel-title">
        ...
        ${pending > 0 ? `<span class="sbadge s-pending">${pending} ${t('pending','زیر التواء')}</span>` : ''}
      </div>
      ${buildButcherRows(bookings, custMap)}
    </div>`;
}

function buildButcherRows(bookings, custMap) {
  if (!bookings || bookings.length === 0) {
    return `<div class="empty">
      <div class="empty-ico">📋</div>
  ...
  return bookings.map(b => {
    ...
    const cname   = cust.full_name || t('Unknown Customer','نامعلوم گاہک');
    const ccity   = cust.city || '';
    ...
    const actions = b.status === 'pending'
      ? `<button class="btn btn-sm" style="background:#dcfce7;color:#15803d"
            onclick="updateStatus('${b.id}','accepted')">✓ ${t('Accept','قبول')}</button>
         <button class="btn btn-sm btn-danger"
            onclick="updateStatus('${b.id}','declined')">✗ ${t('Decline','رد')}</button>`
      : '';
    ...
    const receiptBtn = b.advance_payment_screenshot
      ? `<a href="${b.advance_payment_screenshot}" target="_blank"
            ...
            👁 ${t('Receipt','رسید')}</a>`
      : `<span style="color:var(--ink-muted);font-size:.72rem">${t('No receipt','رسید نہیں')}</span>`;
...
// ── BUTCHER HOME BANNER ──
// Called after login if user is a butcher on the home page
function showButcherHomeBanner() {
  const grid = document.getElementById('butchersGrid');
  if (!grid || currentProfile?.role !== 'butcher') return;
  grid.innerHTML = `
    <div style="grid-column:1/-1;text-align:center;padding:3rem">
      <div style="font-size:3rem;margin-bottom:1rem">🔪</div>
      <h3 style="color:var(--teal-700);margin-bottom:.5rem">
        ${t('Welcome, ', 'خوش آمدید، ')}${currentProfile.full_name?.split(' ')[0] || ''}!
      </h3>
      <p style="color:var(--ink-muted);margin-bottom:1.5rem">
        ${t('You are registered as a butcher. Go to your dashboard to manage bookings and rates.',
            'آپ قصائی کے طور پر رجسٹر ہیں۔ بکنگز اور قیمتیں منظم کرنے کے لیے ڈیش بورڈ پر جائیں۔')}
      </p>
      <button class="btn btn-teal" onclick="goToDashboard()" style="font-size:1rem;padding:.75rem 2rem">
        📊 ${t('Go to My Dashboard', 'میرے ڈیش بورڈ پر جائیں')}
      </button>
    </div>`;
}