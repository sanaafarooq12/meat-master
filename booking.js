/* ═══════════════════════════════════════════
   MEATMASTER — booking.js  v2.1  (FIXED)
   ═══════════════════════════════════════════ */
...
const EID_LABELS_UR = {
  eid_day1: 'عید پہلا دن',
  eid_day2: 'عید دوسرا دن',
  eid_day3: 'عید تیسرا دن',
  normal_day: 'عام دن'
};
...
async function loadButchers() {
  ...
  const { data, error } = await sb
    ...
    .select(`
      *,
      ...
      rates:butcher_rates ( animal_type, price )
    `)
  ...
  if (error || !data || data.length === 0) {
    grid.innerHTML = `<div style="grid-column:1/-1;text-align:center;padding:3rem;color:var(--ink-muted)">
      <div style="font-size:2.5rem;margin-bottom:.75rem">🔪</div>
      <p>${t('No butchers registered yet. Be the first to join!', 'ابھی تک کوئی قصائی رجسٹر نہیں ہوا۔ پہلے بنیں!')}</p>
    </div>`;
    return;
  }
...
function updateBookingSummary() {
  ...
  sumEl.innerHTML = `
    <div class="bsum-row">
      <span>${t('Butcher', 'قصائی')}</span>
      <span>${currentBookingButcher.name}</span>
    </div>
    <div class="bsum-row">
      <span>${t('Animal', 'جانور')}</span>
      <span>${ANIMAL_ICONS[animal]} ${currentLang === 'en' ? ANIMAL_EN[animal] : ANIMAL_UR[animal]} × ${qty}</span>
    </div>
    <div class="bsum-row">
    ...
    <div class="bsum-row">
      <span>${t('Time', 'وقت')}</span>
      <span>${selectedTimeSlot || t('Not selected', 'منتخب نہیں')}</span>
    </div>
    <div class="bsum-row">
      <span>${t('Rate / animal', 'فی جانور قیمت')}</span>
      <span>${unit ? 'PKR ' + unit.toLocaleString() : t('Not set', 'قیمت نہیں')}</span>
    </div>
    <div class="bsum-row total">
      <span>${t('Total Estimate', 'کل تخمینہ')}</span>
      <span>${total ? 'PKR ' + total.toLocaleString() : '—'}</span>
    </div>`;
}
...
async function confirmBooking() {
  ...
  try {
    ...
    // Insert booking — using exact column names from DB
